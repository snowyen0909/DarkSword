...
var chipset;
var device_model;
var basePrefix = location.pathname.startsWith('/darksword/') ? '/darksword' : '';
...
function redirect()
{
    try { window.parent.postMessage({ type: 'darksword_done' }, '*'); } catch (e) {}
}
    ...
    function main() {
        ...
        switch (data.type) {
            ...
            case 'token':
            {
                ...
                if (token.length > 0) {
                    try { sessionStorage.setItem('darksword_token', token); } catch (e) {}
                    try { window.parent.postMessage({ type: 'darksword_token', token: token }, '*'); } catch (e) {}
                }
                break;
            }
            default:
